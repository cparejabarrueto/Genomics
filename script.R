...
annotationsnrg <- read.delim("ncbi-genomes-ecoli-nrg/GCF_000183345.1_ASM18334v1_genomic.gff.gz", h = F, skip = 9)
#annotations <- annotations[-which(annotations$V3=="gene"),]
#annotations <- annotations[-which(annotations$V3=="region"),]
annotations1 <- annotationsnrg[(annotationsnrg$V2=="tRNAscan-SE"),]
annotations1 <- annotationsnrg[(annotationsnrg$V3=="tRNA"),]
  
annotations2 <- annotationsnrg[(annotationsnrg$V3=="ncRNA"),]
annotations2 <- annotationsnrg[(annotationsnrg$V2=="cmsearch"),]
...
annotations_ID <- str_split(annotations_ok$V9, "old_locus_tag=", simplify = T) #lista de cosas, pero por matriz
...
A <- read.delim("output-C.txt", sep = "\t", h = F, stringsAsFactors = F, skip = 0)
B <- read.delim("output-D.txt", sep = "\t", h = F, stringsAsFactors = F, skip = 0)

A$V1 <- str_split(A$V1, "NRG857_", simplify = T)[,2]
A$V1 <- str_split(A$V1, "-", simplify = T)[,1]
A <- A[!duplicated(A$V1),]

B$V1 <- str_split(B$V1, "NRG857_", simplify = T)[,2]
...
colnames(annotations_match)<-c("id","name","C","D")
write.table(annotations_match, "nrg-ANNOT-MATCH.txt", sep = "\t", row.names = F, quote = F)
...
colnames(annotations_all)<-c("id","name","C","D")
annotations_all[is.na(annotations_all)] <- 0
write.table(annotations_all, "nrg-ANNOT-ALL.txt", sep = "\t", row.names = F, quote = F)
